...
At this point, Runtime-Enforcer is up and running. You're now ready to write
some policies.
...
* The proposal includes a list of observed executables for the `opensuse`
  container. As expected, it captured the `ls` and `sleep` commands.
* As the name suggests, this is only a proposal and not a definitive policy yet,
  so nothing is enforced at this stage. To enforce it, we will create a
  `WorkloadPolicy`.
...
`WorkloadPolicyProposal` with the `security.rancher.io/policy-ready` label set
to `true`.
...
if a process other than `ls` or `sleep` is executed, it's reported as an
OTEL event, but it won't be blocked.
...
intentionally left unenforced. See xref:../phases.adoc#monitor-phase[Monitor
phase] and xref:../phases.adoc#protect-phase[Protect phase] for details.
====
...
the workload pods. Since we are using a Deployment, the correct approach is to
...
protections ready. To enable fail-open behavior, set `agent.nriFailopen=true`.
====
...
Now try again an allowed binary, nothing should be reported.
...
This time, if running a not permitted binary, you should see not only a report
