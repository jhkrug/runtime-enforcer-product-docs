...
an OTEL event, but it won't be blocked.
Here the proposal is promoted in `monitor` mode but it is still possible to
promote a proposal directly in `protect` mode. In that case the enforcement
would be applied immediately, blocking any disallowed executables from running.
...
and the violations should be updated automatically by Runtime-Enforcer.
